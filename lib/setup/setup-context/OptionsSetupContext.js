...
export default class extends SetupContext {
  ...
  defineExportObject(object) {
    ...
    const data = {};
    const methods = {};
    Object.entries(object).forEach(([name, prop]) => {
      if (isObject(prop) && prop) {
        throw new Error("cannot expose non-primitive-type value");
      } else if (isFunction(prop)) {
        if (isSignal(prop)) throw new Error("cannot expose signal");
        methods[name] = prop;
      } else {
        data[name] = prop;
      }
    });
    this.setupRecords.exportObj = { data, methods };
  }
  ...
  formatOptions(options) {
    ...
    const { setupOptions, behaviorRefs, classAttrs, exportObj } =
      this.setupRecords;
    ...
    if (this.isComponent) {
      ...
      if (isNonNullObject(exportObj)) {
        // 添加必要的行为 wx://component-export
        behaviorRefs.push("wx://component-export");
        const { data, methods } = exportObj;
        const boundMethods = {};
        Object.keys(methods).forEach((name) => {
          boundMethods[name] = function (...args) {
            console.log(this, args);
          };
        });
        formattedOptions.export = function () {
          console.log(this);
          return Object.assign({}, data, boundMethods);
        };
