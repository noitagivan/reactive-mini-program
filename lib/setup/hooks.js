...
export function onBeforeMounted(handler) {
  useActiveSetupContext()?.instanceLifetimeScope?.on("beforemounted", handler);
}
...
export function onMounted(handler) {
  useActiveSetupContext()?.instanceLifetimeScope?.on("mounted", handler);
}
...
export function onUnmounted(handler) {
  useActiveSetupContext()?.instanceLifetimeScope?.on("dispose", handler);
}
