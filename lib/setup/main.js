...
function setupPage(options) {
  ...
  ctx.reset();
  console.log("[[ PAGE ] Formatted Options ]", formattedOptions);
  ...
  Component({
    ...
    methods: {
      ...
      onLoad(opts) {
        ...
        console.log("lifetimes/onLoad", opts, pageId, this);
...
function setupComponent(options) {
  ...
  ctx.reset();
  console.log("[[ COMPONENT ] Formatted Options ]", formattedOptions);
