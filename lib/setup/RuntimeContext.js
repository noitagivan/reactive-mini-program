...
export default class {
  ...
  exposeSetupContext() {
    ...
    if (ctx) {
      ...
      const instanceScope = this.#getInstanceScope();
      return {
        isPage,
        isComponent,
        ...
        scope: this.exposeInstanceScope(),
      };
    }
    return null;
  }
  ...
  runSetup(setup, ctx) {
    ...
    if (isFunction(setup)) {
      const exposed = ctx.expose(this.#getInstanceScope());
      if (exposed.isSettingUpInstance) {
        console.info(
      ...
      ctx.setupReturns = setup(exposed) || {};
    }
    return ctx;
  }
}
