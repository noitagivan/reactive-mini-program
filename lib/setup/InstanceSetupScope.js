...
class InstanceSetupScope extends EventBus {
  #effectScope = null;
  #parentScope = null;
  #instance = null;
  #isRunning = false;
  ...
  attachTo(parentScope, options) {
    ...
    if (this.isPage) {
      this.context.forwardLifeTimeEvent("load", options);
    } else if (this.isComponent) {
      this.context.forwardLifeTimeEvent("attached");
    }
  ...
  stop() {
    if (this.isPage) {
      this.context.forwardLifeTimeEvent("unload");
    } else if (this.isComponent) {
      this.context.forwardLifeTimeEvent("detached");
    }
...
export default function createInstanceSetupScope(instance, configs) {
  const scope = new InstanceSetupScope(instance);
