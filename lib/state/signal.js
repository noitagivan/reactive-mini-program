...
export function emitSignal(signalOrMapping, payload) {
  const signal = CONTEXT.getSignal(signalOrMapping);
...
export function interpretSignal(signalOrMapping, slient = false) {
  const signal = CONTEXT.getSignal(signalOrMapping);
...
export function watchSignals(
  ...
  const effect = () => {
    const oldValues = [];
    const values = signals?.map((signalOrMapping) => {
      const signal = CONTEXT.getSignal(signalOrMapping);
