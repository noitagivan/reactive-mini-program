 ...
 * @template T
 * @param {T} value
 * @returns {[()=>T, (value:T, options: {forced:boolean; shouldBeRetrack:boolean})=>boolean]}
 */
...
export function watchSignals(
  ...
  signals = isSignal(signals)
    ? [CONTEXT.captureSignal(signals)]
    : isParamLessFunction(signals)
    ? [useComputedSignal(signals)]
    : isArrSrc
    ? signals.map((s) =>
        isSignal(s)
          ? CONTEXT.captureSignal(s)
          : isParamLessFunction(s)
          ? useComputedSignal(s)
          : s
      )
    : [];
  ...
  scope.run(() => signals.forEach((signal) => interpretSignal(signal)));
