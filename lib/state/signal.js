...
const CONTEXT = {
  ...
  getSignal(signalOrMapping) {
    return isSignal(signalOrMapping)
      ? signalOrMapping
      : this.MAPPINGS.get(signalOrMapping);
  },
...
export const isWatchable = (source) =>
  isWatchableSignal(source) || isWatchableSignal(CONTEXT.MAPPINGS.get(source));
...
export function protectSignal(signalOrMapping) {
  assertIsNotInWatchingScope(`modidy signal properties`);
  const signal = CONTEXT.getSignal(signalOrMapping);
  signal[IsProtectedSignal] = true;
}

export function trackSignal(signalOrMapping, effect) {
  try {
  ...
  if (!isRunInEffectFreeScope() && isFunction(effect)) {
    ...
    if (isWatchable(signal)) {
      return signal[SignalSource].subscribe((payload) =>
        effect({ ...payload, retrack: CONTEXT.RETRACKFLAG })
      );
    }
  }
  return () => {};
}
export function emitSignal(signal, payload) {
  if (!signal?.[IsProtectedSignal] && signal?.[SignalSource]) {
    if (payload.retrack) CONTEXT.RETRACKFLAG = true;
    signal[SignalSource].value = payload.value;
    CONTEXT.RETRACKFLAG = false;
    return true;
  }
  return false;
}
export function captureSignal(signalOrMapping, slient = false) {
  const signal = CONTEXT.getSignal(signalOrMapping);
...
export function watchSignals(
  ...
  return scope.exposeHanlde(() => ((signals = null), (cb = null)));
}
